  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white rounded-3xl max-w-6xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-4 border-white"
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white p-6 rounded-t-3xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <Bot className="w-8 h-8" />
              <div>
                <h2 className="text-2xl font-bold">AI Agent Dashboard</h2>
                <p className="text-fuchsia-100">Monitor and control automatic market generation</p>
              </div>
            </div>
            <Button
              onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl p-4 border-2 border-teal-200">
              <div className="flex items-center justify-between mb-3">
                ...
                <div
                  className={`w-3 h-3 rounded-full ${agentStatus?.isRunning ? "bg-teal-500 animate-pulse" : "bg-fuchsia-500"}`}
                ></div>
              </div>
              <h3 className="font-semibold text-teal-800 mb-1">Agent Status</h3>
              <p className="text-lg font-bold text-teal-600">{agentStatus?.isRunning ? "RUNNING" : "STOPPED"}</p>
              {agentStatus?.isGenerating && <p className="text-xs text-teal-600 mt-1">Currently generating...</p>}
            </div>

            <div className="bg-gradient-to-br from-cyan-50 to-teal-50 rounded-2xl p-4 border-2 border-cyan-200">
              <Clock className="w-6 h-6 text-cyan-600 mb-3" />
              <h3 className="font-semibold text-cyan-800 mb-1">Next Execution</h3>
              <p className="text-sm font-medium text-cyan-600">
                {agentStatus?.nextExecution ? new Date(agentStatus.nextExecution).toLocaleString() : "Not scheduled"}
              </p>
            </div>

            <div className="bg-gradient-to-br from-fuchsia-50 to-pink-50 rounded-2xl p-4 border-2 border-fuchsia-200">
              <Target className="w-6 h-6 text-fuchsia-600 mb-3" />
              <h3 className="font-semibold text-fuchsia-800 mb-1">Daily Target</h3>
              <p className="text-lg font-bold text-fuchsia-600">{agentStatus?.config?.dailyMarketCount || 5}</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-4 border-2 border-amber-200">
              ...
              <h3 className="font-semibold text-amber-800 mb-1">Success Rate</h3>
              <p className="text-lg font-bold text-amber-600">{getSuccessRate()}%</p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-2xl p-6 border-2 border-gray-200">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button
                ...
                variant="outline"
                className="border-2 border-fuchsia-200 hover:border-fuchsia-300"
              >
                <Zap className="w-4 h-4 mr-2" />
                {agentStatus?.isGenerating ? "Generating..." : "Generate Now"}
              </Button>

              <Button onClick={refreshData} disabled={isLoading} variant="outline" className="border-2 border-cyan-200">
              ...
              <Button
                ...
                disabled={isLoading}
                variant="outline"
                className="border-2 border-fuchsia-200 hover:border-fuchsia-300 text-fuchsia-600"
              >
          ...
          <div className="bg-gray-50 rounded-2xl p-6 border-2 border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Daily Market Count</label>
                <input
                  type="number"
                  min="1"
                  max="20"
                  ...
                  className="w-full p-3 border-2 border-fuchsia-200 rounded-xl focus:ring-2 focus:ring-fuchsia-500 focus:border-transparent"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Execution Hour (UTC)</label>
                <select
                  value={agentStatus?.config?.executionHour || 9}
                  onChange={(e) => updateConfig("executionHour", Number.parseInt(e.target.value))}
                  className="w-full p-3 border-2 border-cyan-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border-2 border-teal-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                />
              </div>
            </div>
          </div>
          ...
          {agentMetrics && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-white rounded-xl border-2 border-fuchsia-200">
                <TrendingUp className="w-6 h-6 text-fuchsia-600 mx-auto mb-2" />
              ...
              <div className="text-center p-4 bg-white rounded-xl border-2 border-cyan-200">
              ...
              <div className="text-center p-4 bg-white rounded-xl border-2 border-amber-200">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="text-fuchsia-600 border-2 border-fuchsia-200"
                  disabled={agentLogs.length === 0}
                >
                  <Trash2 className="w-4 h-4 mr-2" />
                  Clear
              ...
              {agentLogs.length === 0 ? (
                ...
                agentLogs.map((log, index) => (
                  <div key={index} className={`mb-2 ${getStatusColor(log.status)}`}>
                    ...
                    <span className="font-semibold">[{log.status.toUpperCase()}]</span> {log.message}
                    {log.marketsCount && <span className="text-teal-400"> ({log.marketsCount} markets)</span>}
                    {log.error && <div className="text-fuchsia-400 text-xs mt-1 ml-4">Error: {log.error}</div>}
                  </div>
                ))
              )}
            </div>
